...
# MUNGE MER ---------------------------------------------------------------
  ...
  df_mer <- df %>% 
    filter(indicator %in% c("HTS_TST", "TX_NEW", "TX_CURR", "TX_PVLS"),
...
# MERGE -------------------------------------------------------------------
  ...
  plot_mer_stringency <- function(ind_sel, n_countries = 16){
    ...
    df_viz %>% 
      ...
      labs(x = NULL, y = NULL,
           title = "Comparing PEPFAR MER trends aginst the strictness of ‘lockdown style’ policies" %>% toupper,
