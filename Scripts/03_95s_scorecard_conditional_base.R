...
df_whr <- df_viz %>% 
  filter(country %in% c("Brazil", "El Salvador", "Guatemala", "Honduras", "Jamaica", "Nicaragua",
                        "Panama", "T&T"))
df_arp <- df_viz %>% 
  filter(country %in% c("Burma", "Cambodia", "Laos", "Nepal", "Thailand", "Philippines", "Indonesia","PNG",
                        "Tajikistan", "Kazakhstan", "Kyrgyzstan", "India"))

df_war <- df_viz %>% 
  filter(country %in% c("Benin", "Burkina Faso", "Senegal", "Togo", "Ghana", "Liberia", "Mali", "Sierra Leone"))
...
df_war %>%
  ...
  scale_y_reordered() +
  #coord_cartesian(chttp://127.0.0.1:26967/graphics/plot_zoom_png?width=1200&height=778lip = "off") +
  labs(x = NULL, y = NULL) +
  si_style_nolines() +
...
si_save("Graphics/ctip-ou-unaids_plus_epi-2023-WAR.svg", 
        width = 3.2, height = 12)
