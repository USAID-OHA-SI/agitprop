...
df_hts_youth %>% 
 ...
 summary_rows(
  ...
  tab_header(
    title = glue("TP #10: FY23 YOUTH HTS and HTS_POS" %>% toupper())) %>% 
  gt_theme_nytimes() 
```

### TP #11:  VERTICAL TRANSMISSION
...
pmtct_stat_qc %>% 
 ...
 summary_rows(
  ...
  tab_header(
    title = glue("TP #11: FY23 PMTCT STAT N/D" %>% toupper())) %>% 
  gt_theme_nytimes() 
```
...
pmtct_anc_test_change_qc<-df_final %>% 
  ...
  mutate(prct_change = ((`2023`-`2022`)/ `2022`))
...
pmtct_art_qc %>% 
 ...
 summary_rows(
  ...
  tab_header(
    title = glue("TP #11: FY23 PMTCT_ART" %>% toupper())) %>% 
  gt_theme_nytimes() 
```
...
pmtct_vls_qc %>% 
 ...
 summary_rows(
  ...
  tab_header(
    title = glue("TP #11: FY23 PMTCT VLS Among Preg. Women" %>% toupper())) %>% 
  gt_theme_nytimes() 

```

```{r pmtct_eid, echo=TRUE, include = TRUE, warning=FALSE, message=FALSE, fig.align = 'left'}
...
```{r pmtct_eid_table, echo=FALSE, include = TRUE, warning=FALSE, message=FALSE, fig.align = 'left'}
hiv_free_all %>% 
  gt() %>% 
 ...
 grand_summary_rows(
  ...
  tab_header(
    title = glue("TP #11: Infants born HIV free to mothers living with HIV" %>% toupper())) %>% 
  gt_theme_nytimes() 
...
cxca_scrn_gofurther_val %>% 
  rbind(cxca_scrn_val) %>% 
  mutate(ou_qc = case_when(ou_qc == "All other OUs" ~ "All other OUs minus NGA",
  ...
  tab_header(
    title = glue("TP #14: CXCA_SCRN Cumulative (2018-2023)" %>% toupper())) %>% 
  gt_theme_nytimes() 
```
...
cxca_scrn_pos_val %>% 
  ...
  tab_header(
    title = glue("TP #14: CXCA_SCRN_POS and CXCA_TX Cumulative (FY18-23)" %>% toupper())) %>% 
  gt_theme_nytimes() 
```
...
tb_stat_qc %>% 
  ...
  tab_header(
    title = glue("TP #15: TB_STAT, TB_STAT_POS, and TB_ART" %>% toupper())) %>% 
  gt_theme_nytimes() 
```

------------------------------------------------------------------------

## **Health Equity for Priority Populations - Talking Points**
...
kp_prev_val %>% 
  ...
  tab_header(
    title = glue("TP #16: KP_PREV" %>% toupper())) %>% 
  gt_theme_nytimes() 
```

### TP #17: KP PrEP Initiations
...
prep_kp_val %>% 
  ...
  tab_header(
    title = glue("TP #17: KP and AGYW PrEP Initiations" %>% toupper())) %>% 
  gt_theme_nytimes() 

```

### TP #19: USAID AGYW_PREV Total DREAMS 
...
dreams_overall_val %>% 
  ...
  tab_header(
    title = glue("TP #19: USAID AGYW_PREV Total DREAMS programming" %>% toupper())) %>% 
  gt_theme_nytimes() 
```

### TP #21: USAID AGYW_PREV by Economic/Education Disaggregates
...
df_serv_type %>% 
  ...
  tab_header(
    title = glue("TP #21: USAID AGYW_PREV by Economic/Education Disaggs" %>% toupper())) %>% 
  gt_theme_nytimes() 
```
...
ovc_serv_qc %>% 
  ...
  tab_header(
    title = glue("TP #22: OVC_SERV Comprehensive and Preventive" %>% toupper())) %>% 
  gt_theme_nytimes() 

```

```{r ovc_serv_agency_qc, echo=TRUE, include = TRUE, warning=FALSE, message=FALSE, fig.align = 'left'}

ovc_serv_agency_qc <- df_final %>%
   bind_rows(df_final %>% mutate(funding_agency = "PEPFAR")) %>% 
  filter(indicator %in% c("OVC_SERV"),
         standardizeddisaggregate %in% c("Total Numerator"),
         funding_agency %in% c("PEPFAR", "USAID"),
         fiscal_year == metadata_msd$curr_fy)%>%
  mutate(ou_qc = case_when(operatingunit == "Nigeria" ~ "Nigeria",
                           #operatingunit == "Tanzania" ~ "Tanzania",
                           TRUE ~ "All other OUs")) %>% 
...
```{r ovc_serv_agency_tbl, echo=FALSE, include = TRUE, warning=FALSE, message=FALSE, fig.align = 'left'}
ovc_serv_agency_qc %>% 
  #filter(`OU category` != "Total") %>% 
  gt::gt() %>% 
  fmt_number(columns = c(4,5), 
  ...
  tab_header(
    title = glue("TP #22: OVC_SERV USAID Share" %>% toupper())) %>% 
  gt_theme_nytimes() 
...
ovc_hivstat_qc %>% 
  ...
  tab_header(
    title = glue("TP #22: OVC_HIVSTAT_POS" %>% toupper())) %>% 
  gt_theme_nytimes() 
...
qc_gend_lp %>% 
  ...
  tab_header(
    title = glue("TP #23: GEND_GBV" %>% toupper())) %>% 
  gt_theme_nytimes() 
...
ovc_prev_qc %>% 
  ...
  tab_header(
    title = glue("TP #24: OVC_SERV Preventive 10-14" %>% toupper())) %>% 
  gt_theme_nytimes() 
...
qc_hts_lp %>% 
  ...
  tab_header(
    title = glue("TP #53: Local Partner HTS share" %>% toupper())) %>% 
  gt_theme_nytimes() 
