...
# DATE:     2021-12-06
# UPDATED:  
# NOTES:    adapted from agitprop/17b_stringency_mer.R
...
# GLOBAL VARIABLES --------------------------------------------------------
  ...
  curr_qtr_end <- source_info(return = "period") %>% convert_qtr_to_date() %>% as.Date() %m+% months(3) %m-% days(1)
...
# MERGE -------------------------------------------------------------------
  ...
  plot_mer_stringency <- function(ind_sel, n_countries = 16, save = FALSE){
    ...
    if(str_detect({ind_sel}, "MMD") || str_detect({ind_sel}, "^VL"))
    ...
    df_viz <- df_viz %>% 
      ...
      group_by(countryname) %>% 
      fill(indicator, value, .direction = "down") 
    
    v <- df_viz %>% 
      ggplot(aes(date, value, group = countryname), na.rm = TRUE) +
      geom_ribbon(aes(ymin = 0, ymax = value), 
                  alpha = .4, fill = genoa_light, na.rm = TRUE) +
      geom_vline(xintercept = df_dates$date, color = "white") +
      geom_step(color = genoa, na.rm = TRUE) +
      ...
      labs(x = NULL, y = NULL,
           title = "Comparing PEPFAR MER trends against the strictness of ‘lockdown style’ policies" %>% toupper,
    ...
    if(str_detect({ind_sel}, "MMD") || str_detect({ind_sel}, "^VL")){
      v <- v + scale_y_continuous(label = percent_format(1))
    } else {
      v <- v + scale_y_continuous(label = label_number_si()) 
    }
    
    if(save == TRUE){
      file_out <- glue("Graphics/33_covid_qtr_stringency-{ind_sel}.svg")
  ...
  plot_mer_stringency("TX_NEW", save = TRUE)
  plot_mer_stringency("TX_CURR", save = TRUE)
  plot_mer_stringency("HTS_TST_POS", save = TRUE)
  plot_mer_stringency("HTS_TST", save = TRUE)
  plot_mer_stringency("TX_MMD_o3mo_share", save = TRUE)
  plot_mer_stringency("VLC", n_countries = 15, save = TRUE)
