...
# DATE:     2022-08-01
# UPDATED:  2022-08-03
# GOAL:     Communicate percent USAID share & PEPFAR total results & targets 
#           for a subset of indicators
# TYPE:     Infographic with numbers
# NOTE:     Adapted from 2022_06_14_diagram_usaid-pepfar-contribution_targets_MSD
#           Uses targets and results from MSD
#           Often used for contextual visuals
...
ind_list <- list(c("TX_CURR", "TX_MMD3", "TX_NEW", "TB_PREV","HTS_TST_POS"),
            c("KP_PREV", "OVC_SERV","VMMC_CIRC","PrEP_NEW"))  
...
#tweaked for others
ind_list_plot <- list(c("TX_CURR", "TX_MMD3", "TX_NEW", "TB_PREV","HTS_POS"),
                      c("KP_PREV", "OVC_SERV","VMMC_CIRC","PrEP_NEW"))
...
agency_summary <- function(df_full, region, FY, res_or_tar, ind_vec, ind_vec_plot) {
  
    #Creates indicator for MMD3 by adding together 3mo & 6mo dispensing
    df_full_mmd<-df_full %>% filter(indicator == "TX_CURR", fiscal_year == FY,
    ...
    mutate(indicator=case_when(indicator=="TX_PVLS" ~ "TX_PVLS (N)",
                               ...
                               indicator=="HTS_TST_POS" ~ "HTS_POS",
                               TRUE ~ indicator)) %>%
    bind_rows(df_full_mmd) %>%
    ...
    #Attaches two dummy data sets to allow for calculating total PEPFAR targets
    ...
    summarise(across(c(res_or_tar), sum, na.rm = TRUE), .groups = "drop")
    
    #Pivot wide to calculate share
    df_region_wide <- df_region %>% 
    pivot_wider(names_from=funding_agency, values_from=res_or_tar) %>%
...
reshape_agency_result <- function(df_full, region, FY, res_or_tar, ind_vec, ind_vec_plot) {
   
  df_all <- agency_summary(df_full, region, FY, res_or_tar, ind_vec, ind_vec_plot)
  ...
  df_viz <- df_long %>% 
    mutate(x = .5,
           ...
           ind_display = case_when(indicator == "TX_CURR" ~ "Antiretroviral  treatment (ART) for women, men, and children",
                                   ...
                                   indicator == "HTS_POS" ~ "People who received HIV Testing Services (HTS) & positive test results",
                                   ...
                                   indicator == "AGYW_PREV" ~ "TBD"))
  return(df_viz)
}

#Creates the indicator plot
plot_indicator<- function (df_viz, color_set, res_or_tar) {
  
  if (res_or_tar=="cumulative"){
     res_or_tar<-"results"}
  ...
  plot_ind<-df_viz %>%  
    ...
    labs(x = NULL, y = NULL,
         title = glue ("USAID CONTRIBUTIONS TO {region_sel} PEPFAR {res_or_tar} FY{substr(FY_sel,3,4)}") %>% toupper(),
...
for (i in 1:2) {
df_viz <- reshape_agency_result(df_full, region_sel, FY_sel, res_or_tar, ind_list[[i]], ind_list_plot[[i]]) %>%
...
plot_indicator(df_viz, color_list[i], res_or_tar)
si_save(glue("Images/usaid_contribution_FY{substr(FY_sel,3,4)}_{res_or_tar}_{region_sel}_set{i}.png"))
}
